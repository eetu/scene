//! Edge-trust auth. The party archive is a single shared, read-only collection —
//! there's no per-user state — so the binary doesn't run its own login. It sits
//! behind oauth2-proxy forward-auth and only asserts that the edge vouched for
//! the request via `X-Auth-Request-User`, returning 401 if absent. Bypassed by
//! `DEV_AUTH=1` (local work) or `PARTY_OPEN=1` (a LAN-only deploy). `/status`
...
mod tests {
    ...
    fn state(dev_auth: bool) -> AppState {
        let cfg = Config {
            ...
            transcoder_url: None,
            transcoder_token: None,
        };
        AppState::new(
            cfg,
            Db::open_in_memory().unwrap(),
            PartyConfigs::load(&PathBuf::from("/nonexistent")),
        )
    }
